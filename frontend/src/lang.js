const strings = {
  en: {
    // Tabs
    tabRecords: 'Records',
    tabTimers: 'Timers',

    // Records page
    recordsTitle: 'Details Record',
    addRow: '+ Add Row',
    clearAll: 'Clear All',
    clearConfirm: 'Clear all rows? This cannot be undone.',
    deleteConfirm: 'Delete this row?',
    deleteBtn: 'Delete',
    searchPlaceholder: 'Search...',
    emptyState: 'No records yet.',
    emptyStateSub: 'Tap "Add Row" to get started.',
    noMatch: 'No rows match your search.',
    summaryRows: (n) => `${n} row${n !== 1 ? 's' : ''}`,
    summaryTotal: (amt) => `Total: $${amt}`,
    colActions: 'Actions',
    showCountdown: 'Time left',
    overdue: 'Overdue',
    countdownDaysHrs: (d, h) => `${d}d ${h}h left`,
    countdownHrsMins: (h, m) => `${h}h ${m}m left`,
    countdownMins: (m) => `${m}m left`,
    manageColumns: 'Manage Columns',
    addColumn: 'Add Column',
    deleteColumnConfirm: 'Delete this column? Data in this column will be lost.',
    addField: '+ Field',
    fieldName: 'Field name',
    fieldValue: 'Value',

    // Timers page
    appTitle: 'Timers',
    notificationsOn: 'Notifications on',
    enableNotifications: 'Enable Notifications',
    langToggle: '中文',
    warnNotifications: 'Enable notifications so you get alerted when a timer finishes, even with this tab closed.',
    formTitle: 'New Timer',
    namePlaceholder: 'Timer name',
    hr: 'hr',
    min: 'min',
    sec: 'sec',
    startTimer: 'Start Timer',
    timerAdded: 'Timer added!',
    errorNoName: 'Please enter a timer name.',
    errorNoDuration: 'Please set a duration greater than 0.',
    active: 'Active',
    completed: 'Completed',
    noActive: 'No active timers.',
    done: 'Done',
    deleteLabel: (name) => `Delete ${name}`,
    errorBackend: 'Cannot reach the backend. Is the server running?',
    errorCreate: 'Failed to create timer.',
    installTitle: 'Install on iPhone to enable notifications',
    installStep1: 'Open this page in',
    installStep1b: 'Safari',
    installStep2: 'Tap the',
    installStep2b: 'Share',
    installStep2c: 'button',
    installStep3: 'Choose',
    installStep3b: 'Add to Home Screen',
    installStep4: 'Open the app from your Home Screen',
    installStep5: 'Tap',
    installStep5b: 'Enable Notifications',
    installStep5c: 'above',
    installNote: 'iOS only supports push notifications for installed PWAs.',
  },

  zh: {
    // Tabs
    tabRecords: '明细',
    tabTimers: '计时器',

    // Records page
    recordsTitle: '明细记录',
    addRow: '+ 添加行',
    clearAll: '清空全部',
    clearConfirm: '清空所有行？此操作不可撤销。',
    deleteConfirm: '删除此行？',
    deleteBtn: '删除',
    searchPlaceholder: '搜索...',
    emptyState: '暂无记录。',
    emptyStateSub: '点击"添加行"开始。',
    noMatch: '没有匹配的行。',
    summaryRows: (n) => `${n} 行`,
    summaryTotal: (amt) => `合计：¥${amt}`,
    colActions: '操作',
    showCountdown: '剩余时间',
    overdue: '已过期',
    countdownDaysHrs: (d, h) => `还剩${d}天${h}小时`,
    countdownHrsMins: (h, m) => `还剩${h}小时${m}分`,
    countdownMins: (m) => `还剩${m}分钟`,
    manageColumns: '管理列',
    addColumn: '添加列',
    deleteColumnConfirm: '删除此列？该列数据将丢失。',
    addField: '+ 字段',
    fieldName: '字段名',
    fieldValue: '值',

    // Timers page
    appTitle: '计时器',
    notificationsOn: '通知已开启',
    enableNotifications: '开启通知',
    langToggle: 'English',
    warnNotifications: '请开启通知，这样即使关闭了标签页，计时结束时也能收到提醒。',
    formTitle: '新建计时器',
    namePlaceholder: '计时器名称',
    hr: '时',
    min: '分',
    sec: '秒',
    startTimer: '开始计时',
    timerAdded: '已添加！',
    errorNoName: '请输入计时器名称。',
    errorNoDuration: '请设置大于 0 的时长。',
    active: '进行中',
    completed: '已完成',
    noActive: '暂无计时器。',
    done: '完成',
    deleteLabel: (name) => `删除 ${name}`,
    errorBackend: '无法连接后端，服务器是否正在运行？',
    errorCreate: '创建计时器失败。',
    installTitle: '请在 iPhone 上安装以启用通知',
    installStep1: '在',
    installStep1b: 'Safari',
    installStep2: '点击',
    installStep2b: '分享',
    installStep2c: '按钮',
    installStep3: '选择',
    installStep3b: '添加到主屏幕',
    installStep4: '从主屏幕打开应用',
    installStep5: '点击上方的',
    installStep5b: '开启通知',
    installStep5c: '',
    installNote: 'iOS 仅支持已安装到主屏幕的 PWA 接收推送通知。',
  },
};

export default strings;
